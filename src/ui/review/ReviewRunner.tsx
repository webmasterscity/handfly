        ...
        <div className="flex flex-col gap-3">
          {attempt.trim() && (
            <p className="rounded-xl bg-panel-2 px-4 py-3 text-sm">
              <span className="text-ink-dim">{t('review.yourAttempt')} </span>
              {attempt}
            </p>
          )}
          ...
          <div className="grid grid-cols-2 gap-2">
            {ratings.map(({ r, key, cls }) => (
              <button
                ...
                disabled={busy}
                onClick={() => grade(r)}
                className={`flex min-h-14 flex-col items-center justify-center rounded-xl border-2 bg-panel px-2 font-display font-bold ${cls}`}
              >
